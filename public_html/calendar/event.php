...
// MAIN
$display = '';
$action = Geeklog\Input::fRequest('action', '');

switch ($action) {
    case 'addevent':
        if (($_CA_CONF['personalcalendars'] == 1) && !COM_isAnonUser()) {
            $eid = Geeklog\Input::fGet('eid');
            if (!empty($eid)) {
    ...
    case $LANG_CAL_1[45]: // save edited personal event
        if (!empty($LANG_CAL_1[45]) && ($_CA_CONF['personalcalendars'] == 1) &&
            !COM_isAnonUser() &&
            (Geeklog\Input::post('calendar_type') === 'personal') && SEC_checkToken()
        ) {
    ...
    default:
        $mode = Geeklog\Input::fGet('mode', '');
        $eid = Geeklog\Input::fGet('eid', '');
        $query = Geeklog\Input::fGet('query', '');
        if (!empty($eid)) {
            if (($mode === 'personal') && ($_CA_CONF['personalcalendars'] == 1) && !COM_isAnonUser()) {
            ...
            $day = (int) Geeklog\Input::fGet('day', 0);
