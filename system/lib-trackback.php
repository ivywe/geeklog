...
function TRB_linksToUs($sid, $type, $urlToGet)
{
    ...
    $req = new HTTP_Request2($urlToGet, HTTP_Request2::METHOD_GET);
    ...
    try {
        ...
        if ($status == 200) {
            ...
            COM_errorLog("Trackback verification: Got HTTP response code {$status}"
                . " when requesting {$urlToGet}");
        }
    } catch (HTTP_Request2_Exception $e) {
        COM_errorLog("Trackback verification: " . $e->getMessage()
            . " when requesting $urlToGet");
    }
